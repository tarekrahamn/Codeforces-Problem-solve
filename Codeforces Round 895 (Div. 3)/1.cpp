...
bool prime1(int n){	for(int i = 2; i <= sqrt(n) ; i++){ if(n % i == 0) return false; } return true;}
bool prime(char a, char b){	string s = ""; s += a , s += b;	int x = stoi(s); return prime1(x); }
...
// prefix XOR .we need XOR values for b and c
void XOR(vector<int> &x, vector<int> &a, string &s, int &b, int &c) {
    x[0] = 0;
     fo(i, 0, a.sz) {
        x[i + 1] = x[i] ^ a[i];
        if (s[i] == '0') {
...
void Q(vector<int> &x, string &s,int b,int c){
    int q;
    cin >>q;
    while (q--) {
        int p;
        ...
        if (p == 1) {
            int l, r;
            cin >> l >> r;
            int newB = b ^ x[r] ^ x[l - 1];
            int newC = c ^ x[r] ^ x[l - 1];
            b = newB;
            c = newC;
        } else {
            int l;
