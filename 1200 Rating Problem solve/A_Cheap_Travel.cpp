...
void run(){
    int n,m,a,b;
    cin>>n>>m>>a>>b;
    if(m*a <= b){  // m*a  taka diya ticket kinle ki br chaya kom or eqal taka lage
        cout<<n*a<<"\n";
    }else{
        // 5 / 2 = 2 and remdinder 5 so we nedd handel
        // n number of dAYS
        // m how many ticikent we can take  at a time 
        //  (n%m) exta ticikets
        //  2*3 + min((1*2),3)
        // 6 + 2 = 8
