...
bool isPossibleToFix(int x,vector<pair<int,int>> a) {
    int n = a.sz;
    int leftLimit = 0, rightLimit = 0;
    fora(i,0,n) {
        leftLimit -= x;
        rightLimit += x;
        leftLimit = max(leftLimit, a[i].first);
        rightLimit = min(rightLimit, a[i].second);
...
void Broken_Heart(int t){
    ...
    cin >> a;
    int low = 0, high = 1e18 + 10;
    while (high - low > 1) {
        int mid = (high + low) >> 1;
        if (isPossibleToFix(mid, a)) {
            high = mid;
        } else {
            low = mid;
        }
    }
    if (isPossibleToFix(low, a)) {
        high = low;
    }
