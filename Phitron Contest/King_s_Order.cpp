...
int findingp(int u, vector<int>& parent) { 
    if (u == parent[u]) {
        return u;
    }
    return parent[u] = findingp(parent[u], parent); 
}
void need(int u, int v, vector<int>& parent) {
    u = findingp(u, parent);
    v = findingp(v, parent);
    if (u != v)
        parent[u] = v;
...
void run(int) {
    ...
    fo(i,0,e){
        ...
        int q = ed[i].v;
        int r = ed[i].w; 
        if (findingp(p, parent) != findingp(q, parent)) {
            need(p, q, parent);
            cost += r;
        } else {
            rem++;
