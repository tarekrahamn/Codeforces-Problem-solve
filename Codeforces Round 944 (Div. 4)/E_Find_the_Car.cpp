...
void Broken_Heart(int t){
    ...
    while (m--) {
        ...
        //     }
        //    int prev_pos = a[idx];
        //   int next_pos = a[idx + 1];
        //     int  prev_time = b[idx];
        //     int  next_time = b[idx + 1];
        //     int inter = prev_time + (next_time - prev_time) * (q - prev_pos) / (next_pos - prev_pos);
          int idx = lower_bound(a.begin() + 1, a.end(), q) - a.begin();
            int prev_pos = a[idx - 1];
            int prev_time = b[idx - 1];
            int cur_pos = a[idx];
            int cur_time = b[idx];
            int ans = prev_time + (double)(cur_time - prev_time) / (cur_pos - prev_pos) * (q- prev_pos);
            cout << ans << " ";
        }
    }
    cout << nl;
