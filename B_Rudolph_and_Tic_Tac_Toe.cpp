...
int winner(char p, vector<string>& f) {
    bool flag = false; 
    FOR(i, 0, 3) {
        if (f[i][0] == p && f[i][1] == p && f[i][2] == p) {
            flag = true;  
            break;  
        }
    }
    
    if (!flag && f[0][0] == p && f[1][1] == p && f[2][2] == p) {
        flag = true;  // Set flag to true
    }
    FOR(i, 0, 3) {
        if (!flag && f[0][i] == p && f[1][i] == p && f[2][i] == p) {
            flag = true;  
            break;  
        }
    }
    
    if (!flag && f[0][2] == p && f[1][1] == p && f[2][0] == p) {
        flag = true; 
    }
    return flag;  
}
